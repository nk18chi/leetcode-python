...
class Solution:
    ...
    def spiralOrder(self, matrix: List[List[int]]) -> List[int]:
        ...
        def dfs(self, row: int, col: int, directionIndex: int):
            if row < 0 or row >= len(matrix) or col < 0 or col >= len(matrix[row]) or matrix[row][col] == 777:
                return
            ...
            if nextRow < 0 or nextRow >= len(matrix) or nextCol < 0 or nextCol >= len(matrix[nextRow]) or matrix[nextRow][nextCol] == 777:
